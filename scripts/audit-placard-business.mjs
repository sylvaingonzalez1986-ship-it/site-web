...
const modules = {
  'business-entry': `import React from 'react';import {createRoot} from 'react-dom/client';import '/src/app/globals.css';
    ...
    const originalFetch=window.fetch.bind(window);window.fetch=async(url,init={})=>{
      if(String(url).startsWith('/api/arena/placard/energy'))return new Response(JSON.stringify({outstandingCents:snapshot.electricityOutstandingCents,invoices:[],invoiceCount:0,cashCents:snapshot.cashCents,quotes:{},bestGramsPerKwh:null}));if(!String(url).startsWith('/api/arena/placard/commerce'))return originalFetch(url,init);
    ...
    function App(){const [view,setView]=React.useState('management');const onOpenShop=()=>{window.__openedComputer=true;};return React.createElement(React.Fragment,null,React.createElement('button',{'data-audit-market':true,onClick:()=>setView('market')},'Marché'),React.createElement('button',{'data-audit-invoices':true,onClick:()=>setView('invoices')},'Factures'),React.createElement('button',{'data-audit-management':true,onClick:()=>setView('management')},'Gestion'),view==='market'?React.createElement(KqCommerceDesk,{onOpenShop,onOpenTreasury:()=>setView('management')}):React.createElement(KqTreasuryManagement,{key:view,onOpenShop,section:view}));}createRoot(document.getElementById('root')).render(React.createElement(App));`,
...
try {
  ...
  for(const width of [320,390,1280]){
    ...
    await page.click('[data-audit-invoices]');await page.waitForSelector('[aria-label="Factures laboratoire"]');await page.$eval('[aria-label="Factures laboratoire"] li:nth-child(2) button',button=>button.click());await confirm();assert.equal(await page.evaluate(()=>window.__snapshot.business.lab.invoices[1].remainingCents),0);await page.click('[data-audit-management]');await page.waitForSelector('#kq-business-management[open]');
