...
-- Migration 016 added the Heritage action to the historical five-argument
-- overload. Official runs use the six-argument function introduced by 005 so
-- that a completed run and its Flower are persisted atomically.
DROP FUNCTION IF EXISTS public.rpc_kq_update_run_state(
...
BEGIN
  ...
  SET state = p_next_state,
    integrity_code = CASE
      WHEN v_complete THEN p_flower->>'integrityCode'
      ELSE integrity_code
    END,
    status = CASE
