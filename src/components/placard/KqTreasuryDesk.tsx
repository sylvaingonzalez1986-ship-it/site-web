...
const ACCOUNT_GROUPS: { title: string; description: string; accounts: KqTreasuryAccount[] }[] = [
  { title: "Actif", description: "Ce que possède ton entreprise", accounts: ["cash", "vat_reserve", "savings", "crypto_assets", "securities_assets", "equipment", "website", "stock", "prepaid"] },
      ...
      {tab === "invoices" ? <KqTreasuryManagement key="invoices" onOpenShop={onOpenShop} section="invoices" /> : <>
        ...
        {!matching ? <p className={styles.loading} role="status"><LoaderCircle size={22} className={styles.spin} aria-hidden="true" /> Lecture de tes comptes…</p> : state.error ? <div className={styles.error} role="alert"><strong>Les comptes n’ont pas pu être chargés.</strong><p>{state.error}</p><button type="button" className={styles.primary} onClick={() => setRefresh(value => value + 1)}>Réessayer</button></div> : data && report ? <>
          ...
          {tab === "overview" ? <>
            ...
            <div className={styles.cashStrip}><span>TVA réservée <strong>{money(report.cash.reservedVatCents)}</strong></span><span>Épargne <strong>{money(report.cash.savingsCents)}</strong></span><span>Cryptoactifs · coût d’acquisition <strong>{money(report.cash.cryptoCostCents)}</strong></span><span>Titres boursiers · coût d’acquisition <strong>{money(report.cash.securitiesCostCents)}</strong></span><span>Emprunts restant dus <strong>{money(report.cash.loanDebtCents)}</strong></span><span>Disponible après règlement des factures <strong data-negative={report.cash.afterDebtCents < 0}>{money(report.cash.afterDebtCents)}</strong></span></div>
