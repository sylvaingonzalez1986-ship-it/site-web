...
DROP FUNCTION IF EXISTS public.rpc_kq_update_run_state(UUID, UUID, TIMESTAMPTZ, TEXT, JSONB);

CREATE FUNCTION public.rpc_kq_update_run_state(
  p_user_id UUID,
  p_run_id UUID,
...
DECLARE
  ...
  v_complete BOOLEAN;
BEGIN
  IF p_action NOT IN ('roll', 'resolve', 'advance', 'redraw') OR p_next_state IS NULL
  ...
  IF v_complete THEN
    ...
    IF EXISTS (
      SELECT 1 FROM jsonb_each_text(p_flower->'battleStats') AS stat(key, value)
      WHERE key NOT IN ('appearance', 'aroma', 'vigor', 'mastery', 'regularity')
        OR value::INTEGER NOT BETWEEN 35 AND 99
    ) OR jsonb_object_length(p_flower->'battleStats') <> 5
  ...
  IF v_complete THEN
    INSERT INTO public.kq_flowers (
      run_id, owner_id, variety_code, variety_name, quality, traits, combos, battle_stats
    ) VALUES (
      p_run_id, p_user_id, p_flower->>'varietyCode', p_flower->>'varietyName',
...
REVOKE ALL ON FUNCTION public.rpc_kq_update_run_state(UUID, UUID, TIMESTAMPTZ, TEXT, JSONB, JSONB)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_kq_update_run_state(UUID, UUID, TIMESTAMPTZ, TEXT, JSONB, JSONB)
  TO service_role;
