...
-- A dead culture is terminal but is not a harvested cycle. Reuse abandoned so
-- harvest rewards, stock, full-cycle invoices and equipment wear are not granted.
-- Initial card/token consumption and the run history remain unchanged.
-- Death time stays in state.completedAt; completed_at is reserved for harvested runs.
...
DECLARE
  ...
  v_dead BOOLEAN;
BEGIN
  -- Preserve calendar settlement installed by the business-calendar migration.
  ...
  v_dead := COALESCE(p_next_state->'cultureDead' = 'true'::JSONB, FALSE);

  IF v_dead THEN
    IF v_complete IS NOT TRUE OR p_action NOT IN ('resolve', 'advance')
      OR p_flower IS NOT NULL
      OR p_next_state->'harvestGrams' IS DISTINCT FROM '0'::JSONB
      OR jsonb_typeof(p_next_state->'history') IS DISTINCT FROM 'array'
    THEN RAISE EXCEPTION 'kq_invalid_culture_death'; END IF;

    IF (SELECT count(*) FROM jsonb_array_elements(p_next_state->'history') AS stage
        WHERE stage->'total' = '0'::JSONB) < 2
    THEN RAISE EXCEPTION 'kq_invalid_culture_death'; END IF;
  ...
  SET state = p_next_state,
    integrity_code = CASE WHEN v_complete AND NOT v_dead THEN p_flower->>'integrityCode' ELSE integrity_code END,
    status = CASE
      WHEN v_dead THEN 'abandoned'::public.kq_run_status
      WHEN v_complete THEN 'completed'::public.kq_run_status
      ELSE status
    END,
    completed_at = CASE WHEN v_complete AND NOT v_dead THEN now() ELSE completed_at END,
    updated_at = now()
  WHERE id = p_run_id;

  IF v_complete AND NOT v_dead THEN
  ...
  RETURN jsonb_build_object(
    'state', p_next_state,
    'flower', CASE WHEN v_complete AND NOT v_dead THEN to_jsonb(v_flower) ELSE NULL END
  );
END;
$$;
