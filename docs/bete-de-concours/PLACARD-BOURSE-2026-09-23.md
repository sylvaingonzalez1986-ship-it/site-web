...
Les listes d’actions, les positions et les fenêtres d’achat/vente utilisent des logos locaux, référencés par le symbole de cotation dans `src/lib/stock-market-logos.json`. Le manifeste conserve l’URL source de chaque image. La source principale est le [service de logos Financial Modeling Prep](https://site.financialmodelingprep.com/developer/docs/company-image-api). Les fichiers WebP sont normalisés en 96 × 96 pixels, sans modifier les couleurs de la marque. Le fond est blanc, ou vert foncé pour rendre visibles les dessins blancs. Les marges vides excessives des logos sur fond clair sont réduites. Certaines images du fournisseur sont remplacées par les icônes de leurs sites officiels, dont l’URL est conservée dans le manifeste.

Les images sont servies par le site : aucun appel au fournisseur n’est nécessaire depuis le navigateur. Les dimensions du badge restent fixes ; un chargement défaillant affiche le symbole du titre sans gêner les ordres. Les noms d’entreprises restent lisibles à côté du logo. Les indices gardent leurs icônes : aucun logo propre au CAC 40 ou au S&P 500 n’a été confirmé sur les pages officielles consultées ; les logos de leurs gestionnaires ne sont pas utilisés à leur place.


La première livraison couvre 542 actions, dont les 40 du CAC 40. VMRK (Vivmark Residential) conserve son symbole de secours faute d’image actuelle vérifiable. Les logos des anciens groupes ne lui sont pas attribués.

Maintenance : `node scripts/refresh-stock-market-logos.mjs --write` complète les images manquantes du catalogue. `--force` renouvelle également les images existantes ; `--normalize-local` ajuste leurs marges sans appel réseau. Sans ces options, le script sonde douze exemples. Le téléchargement borne taille et durée, valide le format, refuse les images uniformes et signale les doublons suspects avant publication. Le rapport `output/stock-logo-probes/report.json` liste les images indisponibles ; un code de sortie non nul signale cette couverture partielle. Le navigateur continue à utiliser les fichiers locaux déjà validés.

Validation des logos : compilation TypeScript et ESLint ciblé, contrôle de chaque fichier du manifeste, puis audit navigateur aux largeurs 320, 390 et 1280 px. L’audit vérifie le chargement des fichiers locaux dans la liste, les positions et la fenêtre d’ordre, les dimensions des images et le retour au symbole après une erreur de chargement. Un achat confirmé après cette erreur prouve que le logo ne conditionne pas les opérations.
