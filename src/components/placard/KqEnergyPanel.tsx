...
export function KqEnergyPanel({ selectedMode, onModeChange, onQuoteChange, lockedQuote, runId, disabled = false }: {
  ...
  return <section className={styles.panel} aria-label="Électricité du Placard">
    ...
    {quote ? <>
      ...
      <details><summary>Détail par appareil</summary><ul>{quote.lines.map((line) => <li key={line.code}><span>{line.name} · niv. {line.level}</span><b>{(line.wattHours / 1000).toLocaleString("fr-FR", { maximumFractionDigits: 2 })} kWh</b></li>)}</ul><p>0,30 € virtuel / kWh. Seuls les appareils de culture installés comptent. Le temps hors ligne ne change pas le montant.</p></details>
