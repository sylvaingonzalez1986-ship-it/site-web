...
import {
  LOTTERY_CARD_IMAGE_ACCEPT_ATTRIBUTE,
...
type LotteryCardImageUploadProps = {
  value: string;
  onChange: (nextImagePath: string) => void;
};

function formatMaxSizeLabel(): string {
  const maxSizeMb = LOTTERY_CARD_IMAGE_UPLOAD_MAX_BYTES / (1024 * 1024);
  return `${maxSizeMb.toFixed(0)} Mo max`;
}

export function LotteryCardImageUpload({ value, onChange }: LotteryCardImageUploadProps) {
  ...
  const validateClientFile = (file: File): string | null => {
    if (!isSupportedLotteryCardImageMimeType(file.type)) {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch("/api/admin/lottery/cards/upload", {
        method: "POST",
        body: formData,
      });
    ...
    <div className="grid gap-3">
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-charcoal">Illustration carte: clic ou glisser/deposer</p>
          <button
            type="button"
        ...
        <input
          ref={inputRef}
          type="file"
          accept={LOTTERY_CARD_IMAGE_ACCEPT_ATTRIBUTE}
          className="hidden"
      ...
      <div className="grid gap-2 md:grid-cols-[160px_1fr]">
        <div className="relative h-[220px] overflow-hidden rounded border-2 border-[#1a1a1a] bg-white">
          {isRenderableImageSource(value) ? (
            isRemoteImageUrl(value) ? (
              <img
                src={value}
                alt="Apercu illustration carte"
              ...
              <Image src={value} alt="Apercu illustration carte" fill sizes="160px" className="object-cover" />
            )
          ) : (
            <div className="flex h-full w-full items-center justify-center text-xs text-charcoal">Sans image</div>
          )}
        </div>
        <input
          ...
          value={value}
          readOnly
          aria-label="Chemin image carte TCG"
        />
      </div>
