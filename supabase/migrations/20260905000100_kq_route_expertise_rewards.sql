...
ALTER TABLE public.kq_market_sale_receipts
  ADD COLUMN IF NOT EXISTS route_sale_count INTEGER NOT NULL DEFAULT 1 CHECK (route_sale_count > 0),
  ADD COLUMN IF NOT EXISTS expertise_bonus_reputation INTEGER NOT NULL DEFAULT 0 CHECK (expertise_bonus_reputation >= 0);

WITH ranked_receipts AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
      PARTITION BY owner_id, route
      ORDER BY created_at, id
    )::INTEGER AS route_sale_count
  FROM public.kq_market_sale_receipts
)
UPDATE public.kq_market_sale_receipts AS receipt
SET route_sale_count = ranked.route_sale_count
...
BEGIN
  ...
  v_base_reputation_gain := FLOOR((v_option->>'reputationGain')::NUMERIC);
  IF v_payout < 0 OR v_base_reputation_gain < 0 THEN RAISE EXCEPTION 'invalid_market_quote'; END IF;
  ...
  IF p_route IN (
    ...
    END;
  END IF;
  v_reputation_gain := v_base_reputation_gain + v_expertise_bonus_reputation;
