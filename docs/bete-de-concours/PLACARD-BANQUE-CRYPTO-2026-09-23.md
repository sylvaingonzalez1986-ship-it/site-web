...
Une réservation PostgreSQL commune à toutes les instances limite l’actualisation à un cycle de cinq minutes, comprenant le top 100 et jusqu’à 250 actifs détenus sortis du classement, en rotation. Après un échec, une nouvelle tentative est possible au bout d’une minute ; le verrou commun de 30 secondes continue d’empêcher les appels concurrents. Le navigateur relit le cache toutes les minutes et dès le retour sur l’onglet, hors fenêtre de transaction ouverte. Aucun prix de remplacement n’est fabriqué. Un cours de plus de dix minutes suspend les nouveaux ordres sur cet actif ; les positions restent visibles. Un actif sorti du top 100 peut être vendu avec un cours récent, mais plus acheté.
...
La migration `20260923000600_kq_crypto_refresh_recovery.sql` distingue maintenant une publication réussie (cache partagé de cinq minutes) d’un échec (nouvelle tentative après une minute). Elle a été appliquée à la base liée ; le dry-run suivant confirme qu’elle est à jour. Les contrôles de fraîcheur et les permissions restent inchangés. Les échecs de publication et du fournisseur sont désormais journalisés côté serveur avec des motifs filtrés, sans secrets ni données joueur. Le navigateur recharge immédiatement les cours au retour sur l’onglet et indique lorsqu’une actualisation est en cours.
