...
vi.mock("@/lib/supabase/kanab-quest-crypto-backend",()=>({getKqCryptoSnapshot:snapshot,handleKqCryptoAction:action}));
...
const url="http://localhost/api/arena/placard/crypto";
...
describe("crypto API boundaries",()=>{
 ...
 it("loads only current account with private no-store responses",async()=>{const response=await GET(new Request(url+"?userId=victim"));expect(snapshot).toHaveBeenCalledExactlyOnceWith("owner");expect(response.headers.get("cache-control")).toContain("private, no-store");});
 ...
 it("hides database failures and preserves safe business errors",async()=>{action.mockRejectedValueOnce(new Error("[supabase:crypto] private database detail"));const response=await POST(post({}));expect(response.status).toBe(503);expect(JSON.stringify(await response.json())).not.toContain("private database detail");action.mockRejectedValueOnce(new KqCryptoRequestError("This offer has expired."));expect((await POST(post({}))).status).toBe(400);});
});
