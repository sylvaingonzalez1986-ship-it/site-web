-- Rename persistent collection naming from Hemp Heroes -> Kanab Quest in production data.

DO $$
BEGIN
  IF to_regclass('public.lottery_card_collections') IS NOT NULL THEN
  ...
  IF to_regclass('public.lottery_config') IS NOT NULL THEN
    UPDATE public.lottery_config
  ...
  IF to_regclass('public.lottery_card_definitions') IS NOT NULL THEN
  ...
  IF to_regclass('public.cms_pages') IS NOT NULL THEN
    ...
    WHERE title LIKE '%Hemp Heroes 2026%' OR description LIKE '%Hemp Heroes 2026%';

    UPDATE public.cms_pages
    SET sections = (
      SELECT jsonb_agg(
        CASE
          WHEN jsonb_typeof(section_value) = 'object' AND section_value ? 'body'
            THEN jsonb_set(section_value, '{body}', to_jsonb(replace(section_value->>'body', 'Hemp Heroes 2026', 'Kanab Quest')))
          ELSE section_value
        END
