...
import { ArrowDownLeft, ArrowRight, ArrowUpRight, CalendarClock, ChartNoAxesCombined, Check, ChevronLeft, ChevronRight, FileText, LoaderCircle, RefreshCw, Settings2, Wallet } from "lucide-react";
import { formatKqCash } from "@/lib/kanab-quest-equipment";
import { getKqBusinessClock } from "@/lib/kanab-quest-business";
import { KQ_TREASURY_ACCOUNTS, KQ_TREASURY_PERIODS, getKqTreasuryEntryLabel, getKqTreasuryReport, isKqTreasurySnapshot, type KqTreasuryEntry, type KqTreasuryLine, type KqTreasuryPeriod, type KqTreasurySnapshot } from "@/lib/kanab-quest-treasury";
...
const TABS = [
  ...
  { id: "management", label: "Gestion", icon: Settings2 },
] as const;
...
export function KqTreasuryDesk({ onOpenShop, onOpenMarket }: {
  ...
  return <main className={styles.page}>
    <div className={styles.scenery} aria-hidden="true"><Image src="/placard/commerce-office-v2.webp" alt="" fill sizes="100vw" priority /></div>
    <header className={styles.header}><div><small><Wallet size={16} aria-hidden="true" /> Le bureau du Placard</small><h1>La Trésorerie</h1><p>Tes comptes, tes échéances et les décisions de ton entreprise.</p></div><button type="button" className={styles.marketLink} onClick={onOpenMarket}>Aller au Marché <ArrowRight size={17} aria-hidden="true" /></button></header>
    <nav className={styles.tabs} role="tablist" aria-label="Vues de la trésorerie">{TABS.map((item, index) => <button type="button" role="tab" id={`treasury-tab-${item.id}`} aria-controls={`treasury-panel-${item.id}`} aria-selected={tab === item.id} tabIndex={tab === item.id ? 0 : -1} key={item.id} onClick={() => selectTab(item.id)} onKeyDown={event => navigateTab(event, index)}><item.icon size={19} aria-hidden="true" /><span>{item.label}</span></button>)}</nav>
    <section id={`treasury-panel-${tab}`} role="tabpanel" aria-labelledby={`treasury-tab-${tab}`} className={styles.tabPanel} tabIndex={0}>
      {tab === "management" ? <><div className={styles.managementIntro}><h2>Gérer mon entreprise</h2><p>Site internet, hébergement, campagnes publicitaires, domiciliation, TVA et analyses.</p></div><KqTreasuryManagement onOpenShop={onOpenShop} /></> : <>
        ...
        {!matching ? <p className={styles.loading} role="status"><LoaderCircle size={22} className={styles.spin} aria-hidden="true" /> Lecture de tes comptes…</p> : state.error ? <div className={styles.error} role="alert"><strong>Les comptes n’ont pas pu être chargés.</strong><p>{state.error}</p><button type="button" className={styles.primary} onClick={() => setRefresh(value => value + 1)}>Réessayer</button></div> : data && report ? <>
          ...
          {tab === "overview" ? <>
            ...
            <div className={styles.cashStrip}><span>TVA réservée <strong>{money(report.cash.reservedVatCents)}</strong></span><span>Épargne <strong>{money(report.cash.savingsCents)}</strong></span><span>Disponible après règlement des factures <strong data-negative={report.cash.afterDebtCents < 0}>{money(report.cash.afterDebtCents)}</strong></span></div>
