...
try {
 ...
 console.log(JSON.stringify({provider:'CoinMarketCap',stage,passed:true,assets:top.length,firstAssetId:top[0].id,quotedAt:top[0].quotedAt}));
 stage='held-quote';
...
} catch(error) {console.error(`Live CMC ${stage} contract unavailable:`,error instanceof Error?error.message:'unknown');process.exitCode=1;}
