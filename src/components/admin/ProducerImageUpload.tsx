...
import {
  PRODUCER_IMAGE_ACCEPT_ATTRIBUTE,
...
function formatMaxSizeLabel(): string {
  const maxSizeMb = PRODUCER_IMAGE_UPLOAD_MAX_BYTES / (1024 * 1024);
  return `${maxSizeMb.toFixed(0)} Mo max`;
}

export function ProducerImageUpload({ value, onChange }: ProducerImageUploadProps) {
  ...
  const validateClientFile = (file: File): string | null => {
    if (!isSupportedProducerImageMimeType(file.type)) {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch("/api/admin/producers/upload", {
        method: "POST",
        body: formData,
      });
    ...
    <div className="grid gap-3">
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-charcoal">Image producteur: clic ou glisser/deposer</p>
          <button
            type="button"
        ...
        <input
          ref={inputRef}
          type="file"
          accept={PRODUCER_IMAGE_ACCEPT_ATTRIBUTE}
          className="hidden"
      ...
      <div className="grid gap-2 md:grid-cols-[120px,1fr]">
        ...
        <input
          ...
          value={value}
          readOnly
          aria-label="Chemin image producteur"
        />
      </div>
