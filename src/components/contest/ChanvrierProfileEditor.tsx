...
export function ChanvrierProfileEditor({ profile, onSaved, onClose }: { profile: ChanvrierProfile | null; onSaved: (profile: ChanvrierProfile) => void; onClose: () => void }) {
  ...
  const selectLookTab = (tab: typeof LOOK_TABS[number]["code"]) => {
    setLookTab(tab);
    setPortraitView(tab === "outfit" ? "full" : "portrait");
  };
  const changeStep = (next: "look" | "strength") => {
    setStep(next);
    setPortraitView(next === "strength" || lookTab === "outfit" ? "full" : "portrait");
  ...
  return <dialog ref={dialog} className={styles.dialog} data-chanvrier-profile aria-labelledby="chanvrier-title" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }}>
    ...
    <div className={styles.layout}>
      <aside className={styles.portrait} data-view={portraitView}>
      ...
      <section className={styles.controls}>
        ...
        {step === "look" ? <>
          ...
          <nav className={styles.lookTabs} aria-label="Personnaliser l’apparence">{LOOK_TABS.map(tab => <button key={tab.code} type="button" aria-pressed={lookTab === tab.code} onClick={() => selectLookTab(tab.code)}>{tab.label}</button>)}</nav>
        ...
        <footer className={styles.actions}>
          {step === "strength" ? <button type="button" disabled={busy} onClick={() => changeStep("look")}>Mon apparence</button> : null}
          <button type="button" className={styles.primary} disabled={busy || !/^[A-Za-z0-9._-]{3,24}$/.test(draft.nickname.trim())} onClick={() => step === "look" ? changeStep("strength") : void save()}>{busy ? "Enregistrement…" : step === "look" ? "Choisir ma force" : profile ? "Enregistrer mon profil" : "Créer mon chanvrier"}<ArrowRight size={18} /></button>
        </footer>
      </section>
    </div>
  </dialog>;
