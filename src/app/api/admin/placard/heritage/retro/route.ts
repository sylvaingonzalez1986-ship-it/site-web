...
export async function POST(request: Request) {
  ...
  try {
    const preview = await previewKqHeritagePurchaseBatch(cursor);
    const currentFingerprint = buildKqRetroPreviewFingerprint("heritage", cursor, preview);
    if (execute && previewFingerprint !== currentFingerprint) {
      return NextResponse.json({
        error: "La simulation Héritage a changé. Relancez l’aperçu avant toute écriture.",
      }, { status: 409 });
    }
    const result = execute ? await awardKqHeritagePurchaseBatch(cursor) : preview;
    return NextResponse.json({
