...
import { playKqPlayerCard } from "@/lib/supabase/kanab-quest-backend";
...
export async function POST(request: Request, context: { params: Promise<{ runId: string }> }) {
  ...
  const key = `kq_play_card:${session.customerId}:${ip}`;
  const rateLimit = await hitRateLimit({ key, windowSeconds: 60, maxHits: 30 });
  if (!rateLimit.allowed) {
    logRateLimitRejection({
      endpoint: "POST /api/arena/placard/runs/[runId]/cards",
      key,
      ip,
  ...
  try {
    const result = await playKqPlayerCard(session.customerId, runId, cardCode);
