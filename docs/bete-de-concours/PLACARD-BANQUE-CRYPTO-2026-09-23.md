...
Le correctif de reprise automatique du 24 septembre comprend la migration `20260924000200_kq_crypto_refresh_resilience.sql` et les adaptations du fournisseur, du serveur et de l’interface décrites plus haut. La migration a été appliquée le 24 septembre 2026 avec `supabase db push --linked --yes`. Validation : **106 tests Vitest ciblés, 20 groupes PostgreSQL avec les vrais cours publics, contrôle des permissions des 247 migrations, TypeScript, ESLint et compilation de production réussis**. L’audit navigateur complet passe sur 320, 390, 768 et 1280 px, avec le scénario de pause puis reprise automatique des achats et ventes. Les scénarios SQL vérifient notamment 99 cours négociables avec un actif périmé bloqué, la conservation d’un cours stocké plus récent, les délais `Retry-After`, l’absence d’effet d’une ancienne réservation et les permissions du diagnostic privé. La publication manuelle de 14:51 ne constitue pas le déploiement de ce correctif.
...
Contrôle distant après migration à 15:20:12 UTC : le rafraîchissement normal exécuté avec le nouveau backend, sans clé CoinMarketCap, a publié 100 cours frais dans le cache lié (cours horodatés 15:19:03 UTC). Le diagnostic indique zéro échec et le prochain essai à 15:25:12 UTC. Aucun compte joueur ni ordre financier distant n’a été utilisé ; preuve dans `output/placard-crypto-resilience-refresh.json`. Les achats/ventes avec les vrais cours ont été exécutés uniquement en PostgreSQL isolé. La vérification du déploiement web reste distincte de ce contrôle du cache.
