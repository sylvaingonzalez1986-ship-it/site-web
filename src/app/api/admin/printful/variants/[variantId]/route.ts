...
export async function PUT(
  ...
  const publishRateLimitKey = `admin:printful:publish:${adminContext.customerId}`;
  ...
  if (!rateLimit.allowed) {
    logRateLimitRejection({
      endpoint: "PUT /api/admin/printful/variants/[variantId]",
      key: publishRateLimitKey,
      ip,
...
export async function DELETE(
  ...
  const ip = getRequestIp(request);
  const unpublishRateLimitKey = `admin:printful:unpublish:${adminContext.customerId}`;
  const rateLimit = await hitRateLimit({
  ...
  if (!rateLimit.allowed) {
    logRateLimitRejection({
      endpoint: "DELETE /api/admin/printful/variants/[variantId]",
      key: unpublishRateLimitKey,
      ip,
      actorEmail: adminContext.email,
      retryAfterSeconds: rateLimit.retryAfterSeconds,
