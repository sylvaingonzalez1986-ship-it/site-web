...
import { KQ_CHANNELS, KQ_COMPUTER_PRICE_CENTS, KQ_INTERNET_PRICE_CENTS, KQ_SALES_CHANNELS, getKqCommerceReplenishment, quoteKqCommerce, type KqOnlinePrice, type KqSalesChannel } from '@/lib/kanab-quest-commerce';
 ...
 return <dialog ref={dialog} className={styles.dialog} data-arena-journey-ui data-trial-chapter={s.chapter} aria-labelledby="trial-title" onCancel={event=>{event.preventDefault();if(!detail)onPause();}}>
  ...
  <div className={styles.layout}>
   ...
   <main className={styles.main}>
    ...
    {s.chapter===3?<>
     <Image className={styles.scene} src="/placard/warehouse-room-v1.webp" alt="La box, l’établi et la pièce séchoir dans l’entrepôt" width={1100} height={720} sizes="(max-width: 760px) 100vw, 800px"/>
     ...
     <details><summary>Charges, entretien et sécurité</summary><p>L’électricité dépend de ton installation et du mode choisi. Les factures peuvent être prélevées progressivement sur les ventes. Internet coûte 15 € par cycle si tu le reconduis.</p><p>Les machines de transformation s’usent et demandent périodiquement une réparation. Le Bricoleur ne paie pas ces réparations. Les protections limitent les pertes ; le chien a des frais de nourriture et un vétérinaire tous les 10 cycles.</p><p>Le séchoir améliore la régularité et, à certains niveaux, le potentiel de qualité. Il consomme aussi de l’électricité.</p></details>
    </>:null}
    ...
    {s.chapter===8?<>
     <section className={styles.panel}><h3>Ton accès à la vente directe</h3><button type="button" disabled={c.computerOwned} onClick={()=>dispatch({type:'computer'})}>{c.computerOwned?'✓ Ordinateur acheté':`Acheter l’ordinateur · ${euro(KQ_COMPUTER_PRICE_CENTS)} fictifs`}</button><button type="button" disabled={!c.computerOwned||!!c.campaign?.internetPaid} onClick={()=>dispatch({type:'internet'})}>{c.campaign?.internetPaid?'✓ Internet actif':`Activer Internet · ${euro(KQ_INTERNET_PRICE_CENTS)} fictifs`}</button><p>L’abonnement est payé chaque cycle si tu le reconduis. Tu peux vendre aux professionnels sans ordinateur.</p></section>
     {stock?<>
     ...
     {last?<section className={styles.receipt} role="status"><h3>{SATISFACTION[last.satisfaction]}</h3><p>Vente : {euro(last.payoutCents)} · Électricité réglée : {euro(last.electricity)} · Net : <strong>{euro(last.net)}</strong></p><p>Clients : {last.clientsBefore} → {last.clientsAfter} · Shops : {last.shopPartnersBefore} → {last.shopPartnersAfter} · Réputation : {last.reputationDelta>=0?'+':''}{last.reputationDelta}</p><p>Un acheteur satisfait ne crée pas forcément un client ou un shop à chaque petite vente : les volumes et les plafonds de progression comptent.</p></section>:null}
     {stock&&comparison?<details><summary>Essaie aussi un autre contexte de vente</summary><p>Comparaison pédagogique, sans modifier ton lot ni tes ventes. L’ordinateur et Internet sont supposés actifs dans cet aperçu.</p><label>Scénario<select value={scenario} onChange={event=>setScenario(event.target.value)}><option value="normal">Conditions actuelles</option><option value="quality">Qualité faible : 4/10</option><option value="promotion">Concurrents en promotion</option><option value="reputation">Réputation à 1 500</option></select></label><p role="status">{comparison.reason??`${number(comparison.units/10)} g commandés à ${euro(comparison.unitCents)}/g. ${SATISFACTION[comparison.satisfaction]}. Clients : ${comparison.clientsBefore} → ${comparison.clientsAfter}.`}</p><p>Compare le prix et le volume : monter sa réputation n’efface pas une mauvaise qualité, et baisser son prix ne garantit pas de tout vendre.</p></details>:null}
