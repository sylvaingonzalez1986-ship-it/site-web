...
-- Aggregate only; no other player's identity or individual lots are exposed.
CREATE OR REPLACE FUNCTION public.rpc_kq_market_pulse(p_user_id UUID)
RETURNS JSONB LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT jsonb_build_object(
    'marketVolumes', COALESCE((SELECT jsonb_object_agg(route, amount) FROM (
      SELECT route, count(*) AS amount FROM public.kq_market_sale_receipts
      WHERE created_at >= now() - interval '24 hours' AND route <> 'biomass' GROUP BY route
    ) volumes), '{}'::JSONB),
    'recentSales', COALESCE((SELECT jsonb_object_agg(route, amount) FROM (
      SELECT route, count(*) AS amount FROM public.kq_market_sale_receipts
      WHERE owner_id = p_user_id AND created_at >= now() - interval '24 hours' GROUP BY route
    ) recent), '{}'::JSONB)
  );
$$;
REVOKE ALL ON FUNCTION public.rpc_kq_market_pulse(UUID) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_kq_market_pulse(UUID) TO service_role;
CREATE INDEX IF NOT EXISTS kq_market_receipts_recent_route_idx ON public.kq_market_sale_receipts(created_at, route);

CREATE OR REPLACE FUNCTION public.rpc_kq_sell_market_offer(
  p_user_id UUID,
  p_flower_id UUID,
  p_request_key UUID,
  p_route TEXT,
  p_price_policy TEXT,
  p_expected_payout INTEGER,
  p_market_window BIGINT
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF FOUND THEN
    IF v_existing.flower_id <> p_flower_id OR v_existing.route <> p_route
    THEN RAISE EXCEPTION 'invalid_market_sale'; END IF;
  ...
  IF p_price_policy IS NULL OR p_price_policy NOT IN ('clearance', 'fair', 'premium')
    OR p_market_window IS NULL OR p_expected_payout IS NULL
    OR p_market_window <> FLOOR(EXTRACT(EPOCH FROM now()) / 21600)::BIGINT
    OR COALESCE((v_option->'market'->>'window')::BIGINT, -1) <> p_market_window
  THEN RAISE EXCEPTION 'market_offer_changed'; END IF;
  SELECT value INTO v_offer FROM jsonb_array_elements(v_option->'market'->'offers')
  WHERE value->>'policy' = p_price_policy LIMIT 1;
  IF v_offer IS NULL OR NOT COALESCE((v_offer->>'accepted')::BOOLEAN, FALSE)
  THEN RAISE EXCEPTION 'market_no_buyer'; END IF;
  v_payout := FLOOR((v_offer->>'payoutCents')::NUMERIC);
  IF v_payout IS DISTINCT FROM p_expected_payout THEN RAISE EXCEPTION 'market_offer_changed'; END IF;
  ...
  FOR UPDATE;
  IF COALESCE((v_option->'market'->>'reputation')::INTEGER, -1) <> v_reputation_before
  THEN RAISE EXCEPTION 'market_offer_changed'; END IF;
  IF COALESCE((v_option->'market'->>'salesCount')::BIGINT, -1) <> (
    SELECT COALESCE(SUM(sale_count), 0) FROM public.kq_player_route_masteries WHERE user_id = p_user_id
  ) THEN RAISE EXCEPTION 'market_offer_changed'; END IF;
...
REVOKE ALL ON FUNCTION public.rpc_kq_sell_market_offer(UUID, UUID, UUID, TEXT, TEXT, INTEGER, BIGINT) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_kq_sell_market_offer(UUID, UUID, UUID, TEXT, TEXT, INTEGER, BIGINT) TO service_role;


-- Keep the existing app compatible with its own quotes during deployment.
...
BEGIN
  ...
  -- Existing deployed clients may still author legacy quotes. Never let that
  -- endpoint settle a new offer without its explicit price and buyer checks.
  IF v_option ? 'market' THEN RAISE EXCEPTION 'market_offer_changed'; END IF;
