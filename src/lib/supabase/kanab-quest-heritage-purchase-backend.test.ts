...
describe("dormant Kanab Quest Heritage purchase hook", () => {
  it("returns before creating a Supabase client", async () => {
  ...
  it("keeps the paginated purchase retro-attribution dormant", async () => {
    expect(KQ_HERITAGE_RETRO_BATCH_SIZE).toBe(25);
    await expect(awardKqHeritagePurchaseBatch(400)).resolves.toEqual({
      live: false,
      processedItems: 0,
      eligibleUnits: 0,
      awarded: 0,
